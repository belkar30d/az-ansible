...
class Depends(object):
    ...
    def platform_profiles(self):
        ...
        if distro_id in ["debian", "ubuntu", "pop"]:
            atoms.add("dpkg")
            # https://wiki.debian.org/Python
            # https://wiki.ubuntu.com/Python
            # pop os is derived form ubuntu and uses python3
            if (
                (distro_id == 'ubuntu' and release_version >=
                 as_ver("16.04"))
                or (distro_id == 'debian' and (
                    # Treat testing and unstable as newer
...
class Unknown(Platform):
    ...
    def get_pkg_version(self, pkg_name):
        raise Exception("Unknown package manager for current platform.")
